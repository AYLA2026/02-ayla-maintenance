  ...
  if (user) {
    ...
    return (
      <div className="min-h-screen bg-[#1A0F09] flex flex-col items-center justify-center gap-4">
        <p className="text-[#C9A227] font-bold">Welcome, {user.name}</p>
        <button onClick={() => window.location.assign(path)} className="px-6 py-2 bg-[#C9A227] text-[#1A0F09] rounded-xl font-bold text-sm">Enter Platform</button>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#1A0F09] flex items-center justify-center p-4" dir="rtl">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white/5 backdrop-blur border border-[#C9A227]/10 rounded-2xl p-6 space-y-4">
          ...
          <div>
            <label className="text-xs text-[#C9A227]/60 block mb-1.5">Email (optional)</label>
          ...
          <button onClick={handleLogin} disabled={submitting} className="w-full py-3 rounded-xl bg-[#C9A227] text-[#1A0F09] font-bold text-sm hover:bg-[#b89420] transition disabled:opacity-50 flex items-center justify-center gap-2">
