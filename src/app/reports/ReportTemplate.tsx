...
export default function ReportTemplate({ title, color }: Props) {
  ...
  const analyzeImage = (
    ...
    return new Promise((resolve) => {
      ...
      img.onerror = () =>
        resolve({ brightness: 0, contrast: 0, width: 0, height: 0, hasVest: false });
      img.src = url;
    });
  };
  ...
  const smartFilter = useCallback(async () => {
    ...
    const updated = await Promise.all(
      images.map(async (img) => {
        ...
        if (r.brightness < 35) {
          status = "bad";
          reason = "صورة مظلمة جداً";
        } else if (r.brightness > 245 && r.contrast < 15) {
          status = "note";
          reason = "ورقة ملاحظة/بيضاء";
        } else if (r.width < 400 || r.height < 400) {
          status = "bad";
          reason = "دقة منخفضة";
        } else if (r.contrast < 8) {
          status = "bad";
          reason = "صورة ضبابية";
        } else if (r.brightness > 240 && r.contrast < 20) {
          status = "note";
          reason = "احتمال ملاحظة";
        } else if (!r.hasVest && img.type === "before") {
          status = "note";
          reason = "تنبيه: لا يوجد سيفتي فيست واضح";
        }
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F2] p-4 lg:p-6 print:p-0 print:bg-white">
      <div className="max-w-7xl mx-auto print:max-w-none">
        <h1
          className="text-2xl lg:text-3xl font-bold text-[#2C1810] mb-6 flex items-center gap-3"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 print:hidden">
          <button
            ...
            className={`p-4 rounded-2xl border ${c.border} bg-white hover:shadow-md transition flex items-center gap-3`}
          >
            <div
              className={`w-10 h-10 rounded-lg ${c.light} flex items-center justify-center`}
            >
          ...
          <button
            onClick={smartFilter}
            disabled={images.length === 0 || filtering}
            className={`p-4 rounded-2xl border ${c.border} bg-white hover:shadow-md transition flex items-center gap-3 disabled:opacity-50`}
          >
            <div
              className={`w-10 h-10 rounded-lg ${c.light} flex items-center justify-center`}
            >
        ...
        {excelRows.length > 0 && (
          <div className="mb-8 overflow-x-auto print:hidden">
            <h3 className="font-bold text-[#2C1810] mb-3">
              بيانات النموذج المستورد
            </h3>
              ...
              <thead className="bg-[#FAF7F2]">
                <tr>
                  {headers.map((h, i) => (
                    <th
                      key={i}
                      className="px-4 py-3 text-right font-bold text-[#5C3A2A] whitespace-nowrap"
                    >
                      {h || `عمود ${i + 1}`}
                    </th>
                  ))}
            ...
            {excelRows.length > 30 && (
              <p className="text-xs text-gray-400 mt-2">
                + {excelRows.length - 30} صفوف إضافية
              </p>
            )}
          </div>
        )}
        ...
        {images.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6 print:hidden">
            <div
              className={`p-3 rounded-xl ${c.badge} border ${c.border} text-center`}
            >
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
              {images.map((img) => (
                <div
                  key={img.id}
                  className="relative group rounded-xl overflow-hidden border border-[#C9A227]/10 bg-white"
                >
                  <img
                    src={img.url}
                    alt={img.name}
                    ...
                    {img.type && (
                      <span
                        className={`px-1.5 py-0.5 rounded text-[10px] font-bold text-white ${
                          img.type === "before" ? "bg-blue-500" : "bg-green-500"
                        }`}
                      >
          ...
          <div className="flex flex-wrap gap-3 print:hidden">
            {excelRows.length > 0 && (
              <button
                ...
                className={`px-6 py-3 rounded-xl ${c.btn} text-white font-bold flex items-center gap-2 transition disabled:opacity-50`}
              >
            ...
            {images.length > 0 && (
              <button
                onClick={exportPPT}
                disabled={exporting === "ppt" || goodImages.length === 0}
                className="px-6 py-3 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-bold flex items-center gap-2 transition disabled:opacity-50"
              >
            ...
            <button
              onClick={printReport}
              className="px-6 py-3 rounded-xl bg-[#1A0F09] text-[#C9A227] font-bold flex items-center gap-2 hover:bg-[#2C1810] transition"
            >
