      ...
      <div className="max-w-6xl mx-auto print:max-w-none">
        ...
        {excelRows.length > 0 && (
          <div className="mb-8 overflow-x-auto">
            <h3 className="font-bold text-[#2C1810] mb-3">
              بيانات نموذج الإدارة
            </h3>
            <table className="w-full bg-white rounded-2xl border border-[#C9A227]/10 text-sm">
              <thead className="bg-[#FAF7F2]">
                <tr>
                  {excelRows[0]?.map((_: any, i: number) => (
                    <th
                      key={i}
                      className="px-4 py-3 text-right font-bold text-[#5C3A2A]"
                    >
                      عمود {i + 1}
                    </th>
                  ))}
              ...
              <tbody>
                {excelRows.slice(0, 20).map((row: any[], i: number) => (
                  <tr
                    key={i}
                    className="border-t border-[#C9A227]/5"
                  >
            ...
            {excelRows.length > 20 && (
              <p className="text-xs text-gray-400 mt-2">
                + {excelRows.length - 20} صفوف إضافية
              </p>
            )}
          </div>
        )}
        ...
        {images.length > 0 && (
          <div className="grid grid-cols-4 gap-3 mb-6 print:hidden">
            <div
              className={`p-3 rounded-xl ${c.bg} border ${c.border} text-center`}
            >
        ...
        {images.length > 0 && (
          <div className="flex gap-3 print:hidden">
            <button
              onClick={exportCSV}
              className={`px-6 py-3 rounded-xl ${c.btn} text-white font-bold flex items-center gap-2 transition`}
            >
