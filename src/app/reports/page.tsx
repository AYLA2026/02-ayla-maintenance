...
export default function ReportsPage() {
  ...
  return (
    <div className="p-8 min-h-screen" style={{ background: "linear-gradient(135deg, #FAF7F2 0%, #F5E6D3 100%)" }}>
          ...
          <Link key={i} href={report.href}>
            <Card delay={i * 0.1} className="h-full hover:shadow-lg transition-shadow cursor-pointer">
              <div className="flex items-start gap-4">
                <div className="w-14 h-14 rounded-xl flex items-center justify-center shrink-0" style={{ background: "linear-gradient(135deg, #C9A227 0%, #E8D5A3 100%)" }}>
      ...
      {/* مودال الاستيراد - بدون stopPropagation */}
      {showImportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          {/* الخلفية الداكنة */}
          <div className="absolute inset-0 bg-black/50" onClick={closeModal} />
          {/* محتوى المودال - div عادي بدلاً من Card */}
          <div 
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={closeModal} className="text-[#5C3A2A] hover:text-[#2C1810]">
                <X className="w-5 h-5" />
              </button>
            </div>
